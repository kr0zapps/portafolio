...
# 2. Rewrite Tier 1 Cards to Brutalist (No neon) & Fix expanding
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-16">
    ...
    <div class="group relative rounded-2xl bg-white dark:bg-[#030712] border border-[#1a1c1c]/10 dark:border-white/10 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer flex flex-col" onclick="document.querySelectorAll('.cs-content').forEach(el => el.classList.toggle('expanded'))">
        ...
        <!-- Expanded Content -->
        <div class="cs-content w-full bg-[#f9f9f9] dark:bg-[#050b14] border-t border-[#1a1c1c]/5 dark:border-white/5 relative z-20 expanded-default" id="cs-fitgamer">
            <div class="p-8 md:p-10">
                <div class="flex flex-col gap-6">
                    ...
                    <div class="flex gap-8 mt-2 pt-6 border-t border-[#1a1c1c]/10 dark:border-white/10">
                        <div>
                            <div class="text-2xl font-bold text-[#1a1c1c] dark:text-white mb-1 tracking-tight">4.9<span class="text-sm text-gray-400 ml-1">/5</span></div>
                            <div class="text-[10px] font-mono font-bold tracking-widest text-gray-500">PLAY STORE</div>
                        </div>
                        <div>
                            <div class="text-2xl font-bold text-[#1a1c1c] dark:text-white mb-1 tracking-tight">60<span class="text-sm text-gray-400 ml-1">fps</span></div>
    ...
    <div class="group relative rounded-2xl bg-white dark:bg-[#030712] border border-[#1a1c1c]/10 dark:border-white/10 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer flex flex-col" onclick="document.querySelectorAll('.cs-content').forEach(el => el.classList.toggle('expanded'))">
        ...
        <!-- Expanded Content -->
        <div class="cs-content w-full bg-[#f9f9f9] dark:bg-[#050b14] border-t border-[#1a1c1c]/5 dark:border-white/5 relative z-20 expanded-default" id="cs-spam">
            <div class="p-8 md:p-10">
                <div class="flex flex-col gap-6">
                    <div>
                        <h4 class="font-mono text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">El Problema</h4>
                    ...
                    <div class="flex gap-8 mt-2 pt-6 border-t border-[#1a1c1c]/10 dark:border-white/10">
                        <div>
                            <div class="text-2xl font-bold text-[#1a1c1c] dark:text-white mb-1 tracking-tight">99.8<span class="text-sm text-gray-400 ml-1">%</span></div>
                            <div class="text-[10px] font-mono font-bold tracking-widest text-gray-500">PRECISIÓN</div>
                        </div>
                        <div>
                            <div class="text-2xl font-bold text-[#1a1c1c] dark:text-white mb-1 tracking-tight"><5<span class="text-sm text-gray-400 ml-1">ms</span></div>
...
# 3. Add the "Any landing page" card to Tier 2
old_tier2 = r'(<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">.*?)(</div>\s*</section>)'

# Let's write a small regex to insert the new card inside the Tier 2 grid
tier2_new_card = '''
        <!-- Landing Page CTA Card -->
        <div class="group bg-white dark:bg-[#030712] p-8 rounded-2xl border border-[#1a1c1c]/10 dark:border-white/10 hover:border-[#1a1c1c]/30 dark:hover:border-white/30 transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-md cursor-pointer">
            <div>
...
'''
html = re.sub(old_tier2, r'\1' + tier2_new_card + r'\2', html, flags=re.DOTALL)
